...
class TraceSession:
    ...
    async def measure(
        ...
        handle = await self.begin_step(
            name, inputs, run_type=run_type, metadata=metadata
        )
        try:
    ...
    async def start(self, inputs: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.root = RunTree(
                ...
                or os.getenv("LANGCHAIN_PROJECT", "open-webui-knowledge-pipe"),
    ...
    async def step(
        ...
        snapshot_outputs.setdefault(
            "timing_status",
            "skipped" if snapshot_outputs.get("skipped") else "snapshot",
        )
        await self.end_step(
            handle, snapshot_outputs, error=error, usage_metadata=usage_metadata
        )
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        TOP_K: int = Field(
            default=8, description="Number of Knowledge Base chunks to retrieve."
        )
        MAX_CONTEXT_CHARS: int = Field(
            default=24000, description="Maximum evidence context sent to Tara Ops."
        )
        MIN_SOURCES: int = Field(
            default=1, description="Minimum validated sources before web fallback."
        )
        ...
        WEB_SEARCH_ENGINE: str = Field(
            default="duckduckgo", description="Open WebUI web search engine."
        )
        WEB_SEARCH_RESULT_COUNT: int = Field(
            default=5, description="Maximum web candidates to validate."
        )
        ...
        NOVA_PROVIDER: str = Field(
            default="google_genai", description="Provider label used in cost traces."
        )
    ...
    async def _emit_status(
        ...
        **details: Any,
        ...
        safe_details = {
            ...
            if key
            in {"query", "count", "chunk_count", "source_count", "origin", "error"}
        }
        event = {
            "type": "status",
            "data": {
    ...
    async def _get_gemini_client(self) -> Any:
        ...
        if (
            self._gemini_client_instance is not None
            and self._gemini_client_api_key == api_key
        ):
    ...
    def _gemini_usage(
        ...
        *,
        ...
        prompt_tokens = _as_int(
            _read_value(raw, "prompt_token_count", "promptTokenCount")
        )
        candidates_tokens = _as_int(
            _read_value(raw, "candidates_token_count", "candidatesTokenCount")
        )
        thoughts_tokens = _as_int(
            _read_value(raw, "thoughts_token_count", "thoughtsTokenCount")
        )
        output_tokens = candidates_tokens + thoughts_tokens
        if not output_tokens:
            output_tokens = _as_int(
                _read_value(raw, "response_token_count", "responseTokenCount")
            )
    ...
    def _domain_signals(query: str) -> list[str]:
        ...
        patterns = (
            ...
            (
                "sales_order",
                r"\bsales[ -]?order\b|\bso\b(?=.*\b(?:erp|ids|ticket|offline|order|show|block|submit))",
            ),
    ...
    def _domain_override_signals(cls, query: str) -> list[str]:
        ...
        strong = {name for name, pattern in strong_patterns if re.search(pattern, text)}
        if strong:
    ...
    async def _domain_check(self, query: str) -> dict[str, Any]:
        ...
        try:
            ...
            model = self._setting("DOMAIN_CHECK_MODEL") or self._setting(
                "VALIDATION_MODEL"
            )
            ...
            report = {
                ...
                "greeting_response": (
                    greeting_response[:500] if label == "greeting_only" else ""
                ),
                "raw_response": raw,
                "enabled": True,
                "model": model,
            ...
            if label == "out_of_domain" and override_signals:
                ...
                report["matched_terms"] = sorted(set(matched_terms + override_signals))
                report["reason"] = (
    ...
    def _image_trace_metadata(cls, body: dict[str, Any]) -> dict[str, Any]:
        ...
        for part_index, part in enumerate(non_text_parts, start=1):
            ...
            detail = (
                image_value.get("detail") if isinstance(image_value, dict) else None
            )
            if image_url.startswith("data:"):
                ...
                metadata = {
                    "part_index": part_index,
                    "source_type": (
                        "external_url" if parsed_url.scheme else "relative_url"
                    ),
                    "safe_image_url": (
    ...
    def _trace_request_snapshot(
        ...
        for message in snapshot.get("messages") or []:
            ...
            for part in content:
                ...
                if isinstance(image_value, dict):
                    image_value["url"] = (
                        "<image data omitted; see provider_image_input>"
                    )
                else:
    ...
    async def _analyze_image_for_retrieval(
        ...
        for part_index, part in enumerate(non_text_parts, start=1):
            ...
            try:
                ...
                rejected_images.append({"part_index": part_index, "reason": str(exc)})
...
{query}"""
            ...
            output.update(
                {
                    ...
                    "retrieval_query": (
                        result.get("retrieval_query", query)
                        if include_content
                        else "<content omitted>"
                    ),
                    "raw_response": (
    ...
    async def _retrieve(
        ...
        async def observed_embedding(
            values: Any, prefix: Any = None, user: Any = None
        ) -> Any:
    ...
    @staticmethod
    def _chunk_details(
        chunks: list[dict[str, Any]], include_content: bool = True
    ) -> list[dict[str, Any]]:
        details = []
        for chunk in chunks:
    ...
    def _sources(chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            grouped[str(file_id)]["metadata"].append(
        ...
        return [
            {
                "source": {
                    "id": key if key != "unknown" else None,
                    "name": value["metadata"][0]["name"],
                },
...
EVIDENCE:\n{evidence}"""
        ...
        raw = (response.text or "").strip()
        usage_report = self._gemini_usage(
            response, model=self._setting("VALIDATION_MODEL")
        )
        try:
            decision = json.loads(raw)
        except json.JSONDecodeError:
            decision = {
                "accepted_ranks": [c["rank"] for c in chunks],
                "rejected_ranks": [],
    ...
    def _is_corporate_query(query: str) -> bool:
        ...
        has_rdc = bool(
            re.search(r"\brdc(?:\s+concrete)?\b", query_text)
        ) or "rdcconcrete" in query_text.replace(" ", "")
        corporate_terms = (
            "ceo",
            "chief executive",
            "managing director",
    ...
    def _web_prefilter(candidate: dict[str, Any], query: str) -> Optional[str]:
        ...
        text = " ".join(
            str(candidate.get(field, "")) for field in ("url", "title", "snippet")
        ).lower()
        ...
        if not any(
            term in text for term in ("batch", "silo", "bin", "feed", "concrete", "erp")
        ):
            return "missing_batching_or_erp_marker"
        query_text = query.lower()
        if any(
            term in query_text
            for term in ("silo", "silos", "bin", "bins", "feed", "feeding")
        ) and not any(
            term in text
            for term in ("silo", "silos", "bin", "bins", "feed", "feeding", "feeder")
        ):
        ...
        if not candidates:
            return [], {
                "decision": {
                    "accepted_ranks": [],
                    "rejected_ranks": [],
                    "reason": "No web candidates passed prefilter.",
                },
                "raw_response": "",
            }
...
WEB EVIDENCE:\n{evidence}"""
        ...
        accepted = {
            rank for rank in decision.get("accepted_ranks", []) if isinstance(rank, int)
        }
        validated = [
            candidate for candidate in candidates if candidate["rank"] in accepted
        ]
        return validated, {"decision": decision, "raw_response": raw, **usage_report}
    ...
    async def _web_search(
        ...
        config = getattr(request.app.state, "config", None)
        engine = self._setting("WEB_SEARCH_ENGINE") or self._config_value(
        ...
        async def run_search() -> dict[str, Any]:
            search_batches = await asyncio.gather(
                *(
                    search_web(request, engine, search_query, user)
            ...
            for search_query, batch in zip(queries, search_batches):
                if isinstance(batch, Exception):
                    errors.append(
                        {"query": search_query, "stage": "search", "error": str(batch)}
                    )
                    continue
                    ...
                    "search_results": [
                        {
                            ...
                            if key != "snippet" or include_content
                        }
            ...
            for candidate in search_result["candidates"]:
                ...
                if existing:
                    existing["queries"] = sorted(
                        set(existing.get("queries", []) + [candidate["query"]])
                    )
                else:
            ...
            if urls:
                try:
                    loader = get_web_loader(
                        urls,
                        verify_ssl=self._config_value(
                            config, "ENABLE_WEB_LOADER_SSL_VERIFICATION", True
                        ),
                        requests_per_second=self._config_value(
                            config, "WEB_LOADER_CONCURRENT_REQUESTS", 2
                        ),
                        trust_env=self._config_value(
                            config, "WEB_SEARCH_TRUST_ENV", True
                        ),
                    )
            ...
            for rank, candidate in enumerate(candidates, 1):
                ...
                if not content:
                    prefilter_rejections.append(
                        {**candidate, "rank": rank, "reason": "empty_page_content"}
                    )
                    continue
                ...
                if reason:
                    prefilter_rejections.append(
                        {**candidate, "rank": rank, "reason": reason}
                    )
                    continue
        ...
        validated, validation = await (
            trace.measure(
                ...
                output_builder=lambda result: {
                    ...
                    "accepted_ranks": result[1]
                    .get("decision", {})
                    .get("accepted_ranks", []),
                    "rejected_ranks": result[1]
                    .get("decision", {})
                    .get("rejected_ranks", []),
                    **result[1],
                },
                ...
                metadata=(
                    self._ls_model_metadata(
                        "google_genai", self._setting("VALIDATION_MODEL")
                    )
    ...
    def _build_nova_body(
        self, body: dict[str, Any], context: str, evidence_origin: str
    ) -> dict[str, Any]:
        ...
        original_text, image_parts = self._split_message_content(
        ...
        evidence_instruction = {
            ...
            evidence_origin,
            "Do not claim an evidence origin that is not present in the context.",
        )
        enriched_text = (
            f"{original_query}\n\n"
    ...
    async def _effective_nova_request(
        ...
        if not nova_model:
            raise RuntimeError(
                f"Tara Ops model preset '{self._setting('NOVA_MODEL')}' was not found"
            )
        ...
        if not base_model_id or base_model_id == preset_id:
            raise RuntimeError(
                f"Tara Ops model preset '{preset_id}' does not have a valid base model"
            )
        ...
        params = (
            nova_model.params.model_dump()
            if getattr(nova_model, "params", None)
            else {}
        )
    ...
    def _stream_text(event: Any) -> str:
        ...
        for line in event.splitlines():
            ...
            for choice in payload.get("choices", []):
                ...
                text = delta.get("content") or (choice.get("message", {}) or {}).get(
                    "content"
                )
                if text:
    ...
    def _stream_usage(event: Any) -> dict[str, Any]:
        ...
        input_tokens = _as_int(
            latest.get("input_tokens") or latest.get("prompt_tokens")
        )
        output_tokens = _as_int(
            latest.get("output_tokens") or latest.get("completion_tokens")
        )
    ...
    async def _nova(
        self, request: Any, downstream: dict[str, Any], user: Any
        ...
        await trace.start(
            {
                ...
                "image_analysis_enabled": bool(self._setting("ENABLE_IMAGE_ANALYSIS")),
    ...
    async def _maybe_search_web(
        ...
        try:
            ...
            web_user = (
                UserModel.model_validate(user) if isinstance(user, dict) else user
            )
    ...
    async def _generate_nova_answer(
        ...
        async def build_nova_output(
            ...
            return {
                ...
                "provider_request_had_image": provider_image_input["image_received"],
    ...
    async def _handle_pipe_error(
        ...
        code_match = re.search(r"\b([45]\d{2})\b", error_text)
        status_match = re.search(
            r"\b(INVALID_ARGUMENT|UNAUTHENTICATED|PERMISSION_DENIED|NOT_FOUND|"
            r"RESOURCE_EXHAUSTED|DEADLINE_EXCEEDED|UNAVAILABLE|INTERNAL)\b",
            error_text.upper(),
        )
    ...
    async def pipe(
        ...
        try:
            ...
            retrieval_query = str(image_analysis.get("retrieval_query") or query)
