<script lang="ts">
	...
	const channelHomeHandler = async () => {
		selectedChatId = null;
		selectedFolder.set(null);
		chatId.set('');

		if ($channels?.[0]?.id) {
			await goto(`/channels/${$channels[0].id}`);
		} else {
			await goto('/');
		}
		...
		>
			...
			>
				<a
					...
					href={directChatEnabled ? `${base}/` : channelHomeHref}
					draggable="false"
					on:click={directChatEnabled ? newChatHandler : channelHomeHandler}
				>
					<img
						src="{WEBUI_BASE_URL}/static/favicon.png"
			...
			>
				...
				{#if directChatEnabled && ($models ?? []).length > 0 && (($settings?.pinnedModels ?? []).length > 0 || $config?.default_pinned_models)}
					<Folder
						id="sidebar-models"
				...
				{#if directChatEnabled && $config?.features?.enable_folders && ($user?.role === 'admin' || ($user?.permissions?.features?.folders ?? true))}
					<Folder
						id="sidebar-folders"
